#include<stdlib.h>
#include<stdio.h>

struct Fila{
	int capacidade;
	// o asterisco diz que a variavel p vai ser usada 
	// para conter o endereço de um array de chars.
	//Ou seja, p é um ponteiro.
	
	float *dados;
	int primeiro;
	int ultimo;
	int nItens;
};

void criarFila(struct Fila *f, int c){
	f-> capacidade = c;
	f-> dados = (float*) malloc (f->capacidade * sizeof(float));
	f->primeiro = 0;
	f->ultimo = -1;
	f->nItens= 0;	
}
void inserir (struct Fila *f, int v){
	if(f->ultimos == f->capacidade-1)
	f->ultimo = -1;
	
	f->ultimo++;
	// incrementar ultimo e insere
	f->dados [f->ultimo]=v;
	//mais um item inserido
	f->nItens++;	
}
//pega o item do começo da fila
int remover (struct Fila *f){
	//pega o valor e incrementa o primeiro
	int temp = f->dados[f->primeiros++];
	if (f->primeiro==f->capacidade)
	f->primeiro=0;
	//um item retirado
	f->nItens--;
	return temp;
}

//retorna verdadeiro se a fila está vazia
int estaVazia(struct Fila *f ){
	return (f->nItens==0);
}
//retorna verdadeiro se a fila esta cheia 
int estaCheia (struct Fila *f ){
	return(f->nItens== f->capacidade);
}
//mostrar as informaçoes da fila 
void mostrarFila (struct Fila *f ){
	int cont,i;
	for(cont=0 i= f->primeiro; cont < f->nItens;cont++){
		printf("%.2\t",f->dados[i++);
		if(i==f->capacidade)
		i=0;
	}
	printf("\n\n");
}
void liberarFila(struct Fila *f ){
	free(f);
}

int main () {
	int opcao, capa;
	float valor;
	struct Fila umaFila;

	// cria a fila
	printf("\nCapacidade da fila? ");
	scanf("%d",&capa);
	criarFila(&umaFila, capa);

	// apresenta menu
	while( 1 ){
		printf("\n1 - Inserir elemento");
		printf("\n2 - Remover elemento");
		printf("\n3 - Mostrar Fila");
		printf("\n0 - Sair\n\nOpcao? ");
		
		scanf("%d", &opcao);
		switch(opcao){
			case 0: exit(0);
			case 1: // insere elemento
				if (estaCheia(&umaFila)){
					printf("\nFila Cheia!!!\n\n");
				} else {
					printf("\nValor do elemento a ser inserido? ");
					scanf("%f", &valor);
					inserir(&umaFila,valor);
				}
				break;
			case 2: // remove elemento
				if (estaVazia(&umaFila)){
					printf("\nFila vazia!!!\n\n");
				} else {
					valor = remover(&umaFila);
					printf("\n%1f removido com sucesso\n\n", valor) ; 
				}
				break;
				case 3: // mostrar fila
					if (estaVazia(&umaFila)){
						printf("\nFila vazia!!!\n\n");
					} else {
						printf("\nConteudo da fila => ");
						mostrarFila(&umaFila);
					}
					break;
				default:
					printf("\nOpcao Invalida\n\n");
		}
	}
	liberaFila(&umaFila);
}

